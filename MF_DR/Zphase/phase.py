...
ax2.plot(crossover[:,0],crossover[:,1],color='black',dashes=[3,1],linewidth=2.5,label='Crossover',zorder=1)
ax2.plot(firstorder[:,0],firstorder[:,1],color='black',linewidth=2.5,label='First Order',zorder=1)
...
plt.scatter(297,30,color='r',marker='o',s=20,label=r'CEP',zorder=3)
plt.axis([0.,400.,0.,300.])
# ax2.set_title(r'$M=300\,\mathrm{MeV}$',loc='center')
ax2.set_xlabel(r'$\mu\,[\mathrm{MeV}]$', fontsize=12, color='black')
...
ax2.legend(loc=3,fontsize=9,frameon=False,shadow=True,handlelength=3.,borderpad=0.5,borderaxespad=1,numpoints=1,scatterpoints=1)

fig.subplots_adjust(top=0.9, bottom=0.15, left=0.13, right=0.94, hspace=0.1,wspace=0.1)


fig.savefig("phase1.pdf",transparent=True)