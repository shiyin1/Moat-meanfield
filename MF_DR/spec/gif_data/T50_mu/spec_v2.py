...
# Data for plotting
spec=np.loadtxt('./spec_data_v1/rhoT50mu280.dat')
omega = np.arange(1, 703, 2)
ps = np.arange(1, 499, 2)
spec = np.where((spec > 2.5/10**6), np.nan, spec)
spec1 = np.where((spec < 2.5/10**6), np.nan, spec)
# Create figure
...
#colors = cm.viridis(vnorm(spec))
#colors[spec == 1.5] = cm.autumn(vnorm(spec[spec == 0]))
plt.rcParams['font.size'] = 7
im=ax1.plot_surface(xnew, ynew, spec.T*10**6, rstride=2, cstride=2, cmap="rainbow",edgecolor=(0,0,0,0.3), antialiased=True,linewidth=0.2,norm=vnorm) 
im2=ax1.plot_surface(xnew, ynew, spec1.T*10**6, rstride=2, cstride=2, cmap="gray",edgecolor=(0,0,0,0.3), antialiased=True,linewidth=0.2,norm=vnorm1) 
